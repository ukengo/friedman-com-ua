...
add_action('init', function () {
  ...
  pll_register_string('polylang', 'Учредительные');
...
function get_effective_language() {
    if ( in_array(get_locale(),array('in','en'))) {
        return 'en';
    }
...
function get_conveythis_url() {
    $uri = $_SERVER['REQUEST_URI'];

    // Видаляємо Polylang-префікс (/ua або /ru)
    $cleaned_uri = preg_replace('#^/(ua|ru)(/|$)#', '/', $uri);

    // Забираємо дублі слешів
    $cleaned_uri = ltrim($cleaned_uri, '/');

    // Додаємо префікс /en/ до шляху
    return home_url('/en/' . $cleaned_uri);
}
...
function is_conveythis_page_excluded() {
	$pageUrl = home_url(add_query_arg([], $_SERVER['REQUEST_URI']));
    $rules = get_conveythis_exclusions();
    if (empty($rules)) return false;

    $parsed_url = parse_url($pageUrl);
    $path = $parsed_url['path'] ?? '/';

    // Нормалізація: прибираємо подвійні слеші, слеш в кінці, нижній регістр
    $path = rtrim(strtolower($path), '/');

    foreach ($rules as $rule) {
        $rule_path = rtrim(strtolower(trim($rule['page_url'] ?? '')), '/');
        if (!$rule_path) continue;

        switch ($rule['rule']) {
            case 'contain':
                if (strpos($path, $rule_path) !== false) return true;
                break;
            case 'start':
                if (strpos($path, $rule_path) === 0) return true;
                break;
            case 'end':
                if (substr($path, -strlen($rule_path)) === $rule_path) return true;
                break;
            case 'equal':
                if ($path === $rule_path) return true;
                break;
        }
    }
...
function get_conveythis_exclusions() {
	if ( ! class_exists('ConveyThis') ) return [];

	$conveythis = ConveyThis::Instance();

	// Доступ через Reflection — обхід обмеження private
	$reflection = new ReflectionClass($conveythis);
	if ( ! $reflection->hasProperty('variables') ) return [];

	$property = $reflection->getProperty('variables');
	$property->setAccessible(true);
	$variables = $property->getValue($conveythis);

	// Виклик loadSettingsFromDB(), якщо є
	if ( method_exists($variables, 'loadSettingsFromDB') ) {
		$variables->loadSettingsFromDB();
	}
